...
17. ALIAS PREFIX RULE (MANDATORY): When you assign a table alias, use ONLY the alias to prefix columns. NEVER double-qualify. WRONG: `s.Store.Name`, `c.Customer.CustomerID`, `e.Employee.JobTitle`. CORRECT: `s.Name`, `c.CustomerID`, `e.JobTitle`. If you alias `Sales.Store AS s`, then the Name column is `s.Name` — not `s.Store.Name`. If you alias `Sales.Customer AS c`, the CustomerID is `c.CustomerID` — not `c.Customer.CustomerID`. The alias REPLACES the table name; do not repeat it.
18. CUSTOMER NAME RESOLUTION (MANDATORY): `Sales.Customer` has NO Name column. To display a customer's name, you MUST LEFT JOIN both `Sales.Store s` (via `c.StoreID = s.BusinessEntityID`) and `Person.Person p` (via `c.PersonID = p.BusinessEntityID`), then use: `ISNULL(s.Name, CONCAT(p.FirstName, ' ', p.LastName)) AS CustomerName`. Do NOT double-qualify as `s.Store.Name`. For individual B2C customers only (no stores), a simple `JOIN Person.Person p ON c.PersonID = p.BusinessEntityID` with `CONCAT(p.FirstName, ' ', p.LastName)` is sufficient.
